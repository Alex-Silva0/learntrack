...
})({"76JML":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "46a1393c2c0fb41e";
"use strict";
...
},{}],"cRJ84":[function(require,module,exports,__globalThis) {
...
function onPlayerStateChange(event) {
    if (event.data == YT.PlayerState.PLAYING) {
        ...
        timer = setInterval(()=>{
            ...
            if (pauseAt == -1) shouldPause = hasEnded;
            if (shouldPause) {
                player.pauseVideo();
                createAlternatives();
    ...
    } else if (event.data == YT.PlayerState.ENDED) hasEnded = true;
}
function createAlternatives() {
    ...
    while(alternativesWrapper.firstChild)alternativesWrapper.removeChild(alternativesWrapper.firstChild);
...
function checkAlternative() {
    ...
    if (shouldProceedToNextQuestion) {
        ...
        if (action == "question") createAlternatives();
        else if (action == "video") hideQuestions();
        else {
            const resultsEl = document.getElementById("results");
            if (!resultsEl) return;
            resultsEl.textContent = `Voc\xea acertou ${correctAmount}/${questions.length}.`;
    ...
    if (marked == questions[currentQuestion].correctAnswer) {
        ...
        statusDiv.textContent = "Parab\xe9ns, voc\xea acertou!";
        correctAmount++;
    } else {
        lastMarked.style.backgroundColor = "rgb(255, 82, 82)";
        statusDiv.textContent = "Tente novamente da pr\xf3xima vez";
    }
...
async function main() {
    ...
    try {
        ...
        if (!trackFileId) {
            showErrorAlert("Id n\xe3o recebido");
            return;
        }
        ...
        if (trackFileId.startsWith("r")) createAlternatives();
        else if (sessionInfo.videoid) {
        ...
        showErrorAlert("Erro ao carregar a p\xe1gina");
        console.error(e);
    }
}
function showErrorAlert(msg) {
    (0, _sweetalert2Default.default).fire("Erro", msg, 'error');
}
...
},{"sweetalert2":"1HyFr","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["76JML","cRJ84"], "cRJ84", "parcelRequire94c2")

//# sourceMappingURL=session.2c0fb41e.js.map
